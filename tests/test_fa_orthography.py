...
class TestFinalHeAfterUnprotectedLongA:
    """Word-final ⟨ه⟩ is the consonant /h/ after every spelling of long
    ā, not only after bare ⟨ا⟩.

    The /h/ reading is pinned above for ⟨ماه⟩, where the ⟨اه⟩ unit
    protects it. Two other spellings reach the same long ā and were not
    protected: ⟨آ⟩ is a separate grapheme from ⟨ا⟩, and ⟨خوا⟩ consumes
    the alef before ⟨اه⟩ can match. Both fell through to the
    after-consonant /e/ reading.
    """

    def test_final_he_after_alef_madda_is_h(self):
        assert tr("آه") == "ʔɒːh", tr("آه")

    def test_final_he_after_silent_waw_khwa_is_h(self):
        assert tr("خواه") == "xɒːh", tr("خواه")

    def test_medial_he_after_silent_waw_is_unchanged(self):
        assert tr("خواهر") == "xɒːhɾ", tr("خواهر")


class TestArabicEncodingsOfPersianLetters:
    """Persian ⟨ک⟩ and ⟨ی⟩ each have a second Unicode encoding that
    Persian text in the wild carries.

    U+06A9 KEHEH and U+06CC FARSI YEH are the encodings Persian should
    use, but U+0643 ARABIC LETTER KAF and U+064A ARABIC LETTER YEH are
    visually the same Persian letters and reach Persian text from
    Arabic-layout input. Reading them is not the same claim as endorsing
    them: a grapheme table that omits them DELETES the letter, which is
    strictly worse than any reading it could give.
    """

    def test_arabic_kaf_reads_as_k(self):
        assert tr("كتاب") == "ktɒːb", \
            tr("كتاب")

    def test_arabic_yeh_reads_as_long_i(self):
        assert tr("مرکزي") == "mɾkziː", \
            tr("مرکزي")

    def test_word_initial_arabic_yeh_is_a_glide(self):
        assert tr("يحتمل").startswith("j"), \
            tr("يحتمل")
