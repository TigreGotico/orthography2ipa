...
#: Only the varieties HUMAN-WRITTEN gold attests. Algerian and Libyan were in an earlier
#: version of this change and are dropped — their counts came from machine transcription
#: alone, and label-presence without a second kind of evidence is the standard refused
#: elsewhere in this repository for the homograph letters and for the gaf.
READS = ["ar-TN", "ar-MA"]
NOT_ON_GOLD_EVIDENCE = ["ar-DZ", "ar-LY"]
...
@pytest.mark.parametrize("code", NOT_ON_GOLD_EVIDENCE)
def test_the_varieties_without_gold_do_not_get_it(code):
    """Attested in machine transcription and refused anyway.

    ⟨چ⟩ occurs in Algerian and Libyan rows of the judged corpus, but only in the
    machine-transcribed share; across 1,268,785 human-written gold rows there is none for
    either. A letter supported by nothing but the recogniser belongs in a normalisation
    step, not in a spec — the same line drawn for the near-homograph letters and for
    ⟨گ⟩. Pinned so it is not swept in later on a row count.
    """
    assert "چ" not in get(code).graphemes, f"{code} gained ⟨چ⟩ without gold attestation"
