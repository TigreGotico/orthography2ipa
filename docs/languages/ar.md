...
| Row | n (scored) | short-vowel share of gold characters | baseline PER (board) | folded PER | omission score |
|---|---|---|---|---|---|
| `ar-EG` (`arz`) | 590 | 852/2598 = 24.7 % | 0.3237 | 0.3078 | 0.2304 |
| `ar-SA-x-hejaz` (`acw`) | 1889 | 2979/9240 = 24.4 % | 0.2877 | 0.1983 | 0.2359 |
| `ar-x-gulf` (`afb`) | 614 | 834/2822 = 22.8 % | 0.3786 | 0.2893 | 0.2266 |
| `acm` (Iraqi, scored under its ISO 639-3 code) | 97 | 121/387 = 23.8 % | 0.3666 | 0.3732 | 0.2335 |

Every cell comes from the tree this table ships in. The baseline is the row's
`per` in `benchmarks/results.json`. The share, the folded PER and the omission
score are `scripts/folded_per.py` and `scripts/omission_score.py` run with the
segment set `a i u ə ɪ ʊ ɛ ɔ e o`; the share is the script's own count, removed
characters over gold characters, which is the quantity PER is measured in. The
Iraqi row runs under `acm`: the harness registers that gold under the ISO 639-3
code, and `ar-IQ` is not a name it knows. Its folded PER is above its baseline,
which the script permits: folding the short vowels out of both sides shortens
the gold, so the same consonant errors weigh more.
...
`ar-SA-x-hejaz` lands under the 0.25 target once short vowels are folded out; the
other three stay above it, so the vowel fold explains a large share of the remaining
PER but not all of it. These golds also carry homographs — distinct undiacritized
input skeletons that the same gold file transcribes two or more different ways
(e.g. `arz` أجر → `ʔaɡr`/`ʔadʒdʒar`/`ʔadʒr`/`ʔaɡɡar`; `afb` ال → `ʔɪl`/`ɪl`/`ɪ`) —
evidence that the undiacritized orthography underdetermines these words
independently of the missing short vowels. That is not itself an additional score
floor: the harness scores each skeleton against the best-matching of its gold
transcriptions, so a homograph is scored leniently rather than penalized. The
per-row homograph counts an earlier version of this table carried came from an
older engine and are not reproduced by any shipped script, so they are not
restated here.
