...
**Parent**: none (own language; contributors pt-PT and ext) | **Quality tier**: research | **Glottolog**: no languoid
...
Barranquenho is its own language, modelled like a creole: the spec has no `parent`,
and its ancestors weight the contributors, pt-PT (`lexifier`, 0.6) and Extremaduran
`ext` (`adstrate`, 0.35). The spec inherits pt-PT data only through `graphemes_base`,
