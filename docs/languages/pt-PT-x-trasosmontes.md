...
**Code**: `pt-PT-x-trasosmontes` | **Family**: Indo-European > Romance | **Script**: Latin (alphabet)
...
transmontano-alto-minhoto zone keeps the **four-way apico-alveolar vs laminal
contrast**. Modelled as a positional-grapheme delta on ⟨s⟩ (+ a ⟨ss⟩ grapheme),
leaving the laminal ⟨c, ç, z⟩ series inherited and unmarked (the Mirandese/`mwl`
notation convention):
...
not merged as in standard EP. Cintra (1971:87): *"a permanência da distinção
fonológica em posição inicial de sílaba entre o fonema /tʃ/, representado pelo
grafema ch e o fonema /ʃ/ representado pelo grafema x"*. Modelled as a grapheme
delta (⟨ch⟩ → [tʃ]): `chave → ˈtʃabɨ`. The affricate surfaces in the CLUP gold
(`chaves → ˈtʃaβɨʃ`).
...
`clup_dialect` (`pt-PT-x-trasosmontes`) is a **tiny (n=6), sentence-level** gold
from the U.Porto CLUP archive with undocumented IPA-column provenance, so
