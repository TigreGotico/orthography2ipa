...
| `wikipron_nor` | crowd-scraped | Wiktionary editors | The WikiPron scrape filed under the Norwegian **macrolanguage** code `nor`, because WikiPron sorts an entry by the code inside its `{{IPA|…}}` template rather than by the section heading above it. The words are predominantly Nynorsk, so the row is scored against `nn`. See [Norwegian under the macrolanguage code](#norwegian-under-the-macrolanguage-code-wikipron_nor). |
