...
Run it from the repository root::
    ...
    PYTHONPATH=. python scripts/fold_ha_notation.py
"""
...
def report(limit, label):
    ...
    n_gold_tone = sum(1 for _w, golds, _h in rows
                      if any(any(c in TONE_MARKS
                             ...
                             for g in golds))
    n_gold_length = sum(1 for _w, golds, _h in rows
                        if any(LENGTH_MARK in g for g in golds))
    print(f"{DATASET} / {LANG} ({label}): {len(rows)} words scored, "
          f"{n_gold_tone} with a gold tone mark, "
          f"{n_gold_length} with a gold length mark")
    as_scored, as_exact = per(rows)
    tone_folded, tone_exact = per(rows, fold_tone)
    tone_len, tone_len_exact = per(rows, lambda s: fold_length(fold_tone(s)))
    print(f"  as scored          per={as_scored:.4f} exact={as_exact:.4f}")
    print(f"  tone folded        per={tone_folded:.4f} exact={tone_exact:.4f}")
    print(f"  tone+length folded per={tone_len:.4f} exact={tone_len_exact:.4f}")
    print()
